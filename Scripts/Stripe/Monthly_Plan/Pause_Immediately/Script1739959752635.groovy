...
if (CurrentPlan.contains('Monthly') && ADButton.contains('Deactivate')) {
	...
	WebUI.verifyTextPresent('You cannot deactivate Countable Flex or delete/pause your account because there are active engagements with CMS staff.', false)
    ...
    WebUI.click(findTestObject('Stripe/Pause_Immediately'))

    WebUI.verifyTextPresent('Pause Account Immediately?', false)

    WebUI.verifyTextPresent('you will be charged $19/month until you re-activate your account. Only the Super Admin will be able to re-activate this account', 
        false)

    WebUI.click(findTestObject('Stripe/Proceed_Button'))

    WebUI.verifyTextPresent('Billing Implications for Pausing and Unpausing Subscriptions Mid-Month', false)

    WebUI.verifyTextPresent('Should you decide to pause your subscription in the middle of the month and then resume it, you will incur a pro-rated charge for the remaining days of the month.', 
        false)
    ...
    WebUI.verifyTextPresent('Pause Monthly', false)
    ...
    WebUI.verifyTextPresent('Account Paused', false)
