...
if (CurrentPlan.contains('Essentials Plan')) {
    KeywordUtil.markFailed('User is on Essentials Plan and cannot downgrade to another plan')

    return null //FailureHandling.STOP_ON_FAILURE
} else if ((CurrentPlan.contains('Accelerate Plan') && (downleft > 0)) && (totalengbilling > 50)) {
    WebUI.click(findTestObject('Stripe/Change_Plan'))

    WebUI.delay(3)

    WebUI.mouseOver(findTestObject('Stripe/Essentials_Plan'))

    WebUI.verifyTextPresent('Downgrade to Essentials Plan unavailable', false)

    WebUI.verifyTextPresent('You cannot downgrade to Essentials Plan because your engagements are exceeding more than 50 engagements', 
        false)

    WebUI.delay(2)

    WebUI.mouseOver(findTestObject('Stripe/Accelerate_Plan'))

    WebUI.delay(2)

    WebUI.click(findTestObject('Stripe/Select_Ignite_When_Essentials_Unavailable'))

    WebUI.delay(3)
} else if ((CurrentPlan.contains('Ignite Plan') && (downleft > 0)) && (totalengbilling > 50)) {
    KeywordUtil.markFailed('Cannot downgrade as engagement count is reached the maximum engagements')
...
WebUI.verifyTextPresent('Please be mindful as you have a limit of two downgrade options per year.', false)
...
WebUI.verifyTextPresent('Plan Successfully downgraded!', false)

WebUI.verifyTextPresent('You have successfully downgraded your plan to the Monthly Essentials', false)
