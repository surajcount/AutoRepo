...
WebUI.verifyTextPresent('Days Left', false)
...
String TE = WebUI.getText(findTestObject('Stripe/Total_Engagements')).trim()

int TotEng = TE.toInteger()

println('Engagement Count: ' + TotEng)

WebUI.delay(2)

while (TotEng < maxengcount) {
    if (TotEng == (maxengcount - 1)) {
        WebUI.click(findTestObject('Create_Engagement_Pravin/Engagement_List'))
        ...
        WebUI.verifyTextPresent('You have 1 free engagement remaining to create under free trial, beyond this you won\'t be able to create any further engagement.', 
            false)
        ...
        WebUI.delay(20 //WebUI.setText(findTestObject('Create_Engagement_Pravin/Search_Client'), 'client')
            )
    } else {
        WebUI.click(findTestObject('Create_Engagement_Pravin/Engagement_List'))
    ...
    TE = WebUI.getText(findTestObject('Stripe/Total_Engagements')).trim()
...
WebUI.verifyTextPresent('You have reached the maximum engagement creation limit on your Free Trial', false)

WebUI.click(findTestObject('Stripe/Upgrade_Plan'))
...
if (Math.abs(Total_RHS - Total_RHS_Check) <= tolerance) {
    System.out.println('Test Passed: Total Verified')
} else {
    KeywordUtil.markFailed('Test Failed: Total calculated incorrectly')
}
...
WebUI.verifyTextPresent('You have successfully upgraded your plan to the Monthly Essentials', false)
