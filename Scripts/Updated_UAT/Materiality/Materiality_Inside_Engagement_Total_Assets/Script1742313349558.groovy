...
if (total_assets.contains('(') && total_assets.contains(')')) {
    ...
    total_assets_on_bs = (Integer.parseInt(total_assets) * -1 // Convert to negative number
    )
 ...
 * 
 * //Total Expenses String total_expenses =
 * WebUI.getText(findTestObject('Materiality/Total_Expenses_On_Statement'))
 * 
 * // Check if the string contains parentheses (Negative Number) if
 * (total_expenses.contains('(') && total_expenses.contains(')')) {
 * total_expenses = total_expenses.replaceAll('[^0-9]', '' // Remove all
 * non-numeric characters )
 * 
 * total_expenses_on_stat = (Integer.parseInt(total_expenses) * -1 // Convert to
 * negative number )
 * 
 ...
 * WebUI.comment('Total expenses on statement is : ' + total_expenses_on_stat) }
 * 
 * //Net loss before income tax WebUI.delay(3)
 * 
...
WebUI.click(findTestObject('Materiality/Total_Assets_Option'))
...
if (total_assets_on_bs == CurrentPeriod_Amount) {
	KeywordUtil.markPassed('Total assets fetched correctly')
} else {
	KeywordUtil.markFailed(((('Incorrect total assets value on Balance Sheet ' + total_assets_on_bs) + ' ') + ' and Materiality ') +
...
if (Materiality_cy.contains('(') && Materiality_cy.contains(')')) {
    ...
    WebUI.comment('Materiality cy for total assets value in materiality is : ' + materiality_cy // Remove unwanted characters
        // Convert to positive number
    ...
    WebUI.comment('Materiality cy for total assets in materiality is : ' + materiality_cy)
}
...
WebUI.mouseOver(findTestObject('Materiality/Intended_users_tooltip'))

WebUI.verifyTextPresent('Consider key F/S areas and disclosures, laws/regulations and other matters.', false)
...
WebUI.setText(findTestObject('Materiality/Define_Users_R1'), 'V model of testing ')

WebUI.setText(findTestObject('Materiality/Describe_Factors_R1'), 'Testing is not a stand-alone activity, and it has to adapt the development model chosen for the project.')

WebUI.delay(2)

WebUI.click(findTestObject('Materiality/Add_Row_Intended_Users'))

WebUI.delay(2)

WebUI.setText(findTestObject('Materiality/Define_Users_R2'), 'What is Unit Testing?')

WebUI.setText(findTestObject('Materiality/Describe_Factors_R2'), 'Unit Testing is a type of software testing where individual units or components of a software are tested. The purpose is to validate that each unit of the software code performs as expected. ')
...
WebUI.click(findTestObject('Materiality/Delete_Intended_User_R3'))

WebUI.delay(2)

WebUI.mouseOver(findTestObject('Materiality/Nature_and_Impact_tooltip'))

WebUI.verifyTextPresent('e.g., profitability trends, regulations, particular sensitivities, compliance with loan covenants and users expectations', 
    false)
...
WebUI.setText(findTestObject('Materiality/Describe_Nature_R1'), 'Unit Testing Techniques')

WebUI.setText(findTestObject('Materiality/Describe_Impact_R1'), 'Statement Coverage,Decision Coverage,Branch Coverage,Condition Coverage,Finite State Machine Coverage')

WebUI.delay(2)

WebUI.click(findTestObject('Materiality/Add_Row_Nature_And_Impact'))

WebUI.delay(2)

WebUI.setText(findTestObject('Materiality/Describe_Nature_R2'), 'What is Unit Testing?')

WebUI.setText(findTestObject('Materiality/Describe_Impact_R2'), 'Unit Testing is a type of software testing where individual units or components of a software are tested. The purpose is to validate that each unit of the software code performs as expected. ')
...
WebUI.scrollToElement(findTestObject('Materiality/Delete_Nature_Impact_R3'), 0)

WebUI.click(findTestObject('Materiality/Delete_Nature_Impact_R3'))

WebUI.delay(2)

WebUI.setText(findTestObject('Materiality/Add_Comment'), 'Integration Testing focuses on checking data communication amongst these modules. Hence it is also termed as ‘I & T’ (Integration and Testing), ‘String Testing’ and sometimes ‘Thread Testing’.')
