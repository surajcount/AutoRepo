 ...
 * 
 * if (total_assets.contains('(') && total_assets.contains(')')) { total_assets
...
if (total_revenue.contains('(') && total_revenue.contains(')')) {
    ...
    total_revenue_on_stat = (Integer.parseInt(total_revenue) * -1 // Convert to negative number
    )
...
if (total_revenue_on_stat == CurrentPeriod_Amount) {
    KeywordUtil.markPassed('Total revenue fetched correctly')
} else {
    KeywordUtil.markFailed(((('Incorrect total revenue value on Balance Sheet ' + total_revenue_on_stat) + ' ') + ' and Materiality ') + 
...
if (BenchmarkApplied.contains('(') && BenchmarkApplied.contains(')')) {
    ...
    WebUI.comment('Benchmark applied for gross revenue in materiality is : ' + benchmark_applied // Remove unwanted characters
        // Convert to positive number
    ...
    WebUI.comment('Benchmark applied for gross revenue in materiality is : ' + benchmark_applied)
}
...
if (Materiality_cy.contains('(') && Materiality_cy.contains(')')) {
    ...
    WebUI.comment('Materiality cy for gross revenue in materiality is : ' + materiality_cy // Remove unwanted characters
        // Convert to positive number
    ...
    WebUI.comment('Materiality cy for gross revenue in materiality is : ' + materiality_cy)
}
