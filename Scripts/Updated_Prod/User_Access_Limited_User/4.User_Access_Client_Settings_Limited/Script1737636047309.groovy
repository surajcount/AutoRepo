...
List<Map> elements = driver.findElements(By.xpath('//tr/td[1]'))

int clientcount = elements.size()

print(((' ' + clientcount) + ' ') + 'Client access is provided to limited user')
...
String totClients = WebUI.getText(findTestObject('User_Access/Total_Clients_Access'), FailureHandling.CONTINUE_ON_FAILURE)

int actualClients = Integer.parseInt(totClients.trim())

try {
    assertThat(actualClients).isEqualTo(clientcount)

    WebUI.comment("The total clients match: $actualClients")
}
catch (AssertionError e) {
    WebUI.comment("Mismatch: Expected $clientcount, but got $actualClients")
...
WebUI.setText(findTestObject('Client_All_RegressionCase/AddNewClient_FirstName'), 'Limited')
